...
impl std::fmt::Display for CertType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let guid = match self {
            ...
            CertType::ARK => "00000000-0000-0000-0000-000000000000".to_string(),
        };
...
impl CertTableEntry {
    ...
    pub fn from_guid(guid: &uuid::Uuid, data: Vec<u8>) -> crate::error::Result<Self> {
        let cert_type: CertType = match guid.try_into() {
            Ok(guid) => guid,
            Err(error) => return Err(crate::error::SevSnpError::Unknown),
        };
...
impl CertTableEntryRaw {
    ...
    pub unsafe fn parse_table(
        ...
        loop {
            ...
            entry = *data;
            let guid: Uuid = Uuid::from_slice(entry.guid.as_slice()).unwrap();
